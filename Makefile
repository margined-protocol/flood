...
# # Build each client binary based on its directory
# $(CLIENT_BINARIES): % : | $(BIN)
# 	$(info $(M) Building executable: $@)
# 	$Q $(GO) build \
# 		-tags release \
...
# 	$Q $(GO) build \
# 		-tags release \
#		-ldflags '-X $(MODULE)/cmd/$@/main.Version=blah -X $(MODULE)/cmd/$@/main.BuildDate=blak' \
# 		-o $(BIN)/$(basename $(MODULE)) cmd/$(MODULE)/main.go
